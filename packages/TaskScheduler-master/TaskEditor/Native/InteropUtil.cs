...
namespace System.Runtime.InteropServices
{
	internal static class InteropUtil
	{
		...
		public static T[] ToArray<S, T>(this IntPtr ptr, int count)
		{
			IntPtr tempPtr;
			T[] ret = new T[count];
			...
			for (int i = 0; i < count; i++)
			{
				...
				S val = tempPtr.ToStructure<S>();
				ret[i] = (T)Convert.ChangeType(val, typeof(T));
			}
			return ret;
		}

		public static T[] ToArray<T>(this IntPtr ptr, int count) => ptr.ToArray<T, T>(count);
	}
}
